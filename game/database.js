...
module.exports = class DataBase {
  constructor() {
    ...
    console.log(this.host, this.user, this.database, this.password)
  ...
  getAccountByUsername(user) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT Id, Session, Password FROM accounts where Username = ?', [user]).then((rows) => {
          conn.release()
  ...
  getUserByIdAcc(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT u.rank, u.game_id, u.country FROM users u where u.IdAcc = ?', [id]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
  ...
  getUserByIdAccGame(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn
          .query(
            'SELECT u.Id, u.gm, u.IdAcc, u.country, u.game_id, u.rank, u.gp, u.gold, u.cash, u.name_changes, u.gender, u.photo_url, u.is_muted, u.win, u.loss, ae.head, ae.body, ae.eyes, ae.flag, ae.background, ae.foreground, ac.Session FROM users u INNER JOIN user_avatar_equiped ae ON u.Id = ae.Id INNER JOIN accounts ac ON ac.Id = u.IdAcc WHERE u.IdAcc = ?',
            [id]
          )
          .then((rows) => {
            conn.release()
  ...
  getGuildByIdAcc(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT m.Id, m.Job, g.Name from guild_member m INNER JOIN guild g ON m.Id = g.Id WHERE m.UserId = ?', [id]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
  ...
  getGuildMembersById(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn
          .query(
            'SELECT m.Job, u.game_id, u.IdAcc, u.rank, u.gp, u.gender, u.photo_url from guild_member m INNER JOIN guild g ON m.Id = g.Id INNER JOIN users u ON m.UserId = u.IdAcc WHERE g.Id = ?',
            [id]
          )
          .then((rows) => {
            conn.release()
  ...
  getUserAvatarsByIdAcc(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT Id, UserId, aId, type, is_cash, is_gift, amount, expire_time FROM user_avatars WHERE UserId = ?', [id]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
          ...
          if (rows[0].Session === session) {
            ...
            self
              ...
              .then(function (rows2) {
                ...
                self
                  .getGuildMembersById(data.guild_id)
                  .then(function (rows3) {
                    var res3 = rows3[0]
                    for (var ix in res3) {
                      var data_mem = [res3[ix].IdAcc, res3[ix].game_id, res3[ix].gender, res3[ix].rank, res3[ix].gp, res3[ix].photo_url, 2, 0]
  ...
  getGoldCashByIdAcc(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT gold, cash FROM users WHERE IdAcc = ?', [id]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
  ...
  dontExitsUserByGameId(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT game_id FROM users WHERE LOWER(`game_id`) = ?', [id]).then((rows) => {
          conn.release()
  ...
  dontExitsUserByUsername(username) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT Name, Username FROM accounts WHERE LOWER(`Username`) = ?', [username]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return reject()
          else return resolve()
        })
  ...
  updateGameIdAndCashByIdAcc(nname, discount, user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn
          .query('UPDATE users SET game_id = ?, name_changes = name_changes + 1, cash = cash - ? WHERE IdAcc = ?', [nname, discount, user_id])
          .then((rows) => {
            conn.release()
            if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows)
            else return reject()
  ...
  changeName(nname, account) {
    ...
    return new Promise(function (resolve, reject) {
      self
        ...
        .then(function () {
          var discount = 0
          if (account.player.name_changes > 0) discount = 4000
          if (account.player.cash < discount) {
            data.error_cash = true
          } else {
            account.player.name_changes += 1
            self.updateGameIdAndCashByIdAcc(nname, discount, account.user_id).then(function (rows2) {
              data.change = true
              return resolve(data)
            })
          }
  ...
  dontExistGuildName(name) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT * FROM guild WHERE LOWER(`Name`) = ?', [name]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return reject()
          else return resolve()
        })
  ...
  putGuild(name) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('INSERT INTO guild SET Name = ?', [name]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  putGuildMember(guild_id, user_id, job) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('INSERT INTO guild_member SET Id = ?, UserId = ?, Job = ?', [guild_id, user_id, job]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  updateUserDiscountGoldForGuild(mont, user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('UPDATE users SET gold = gold - ? WHERE IdAcc = ?', [mont, user_id]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows)
          else return reject()
        })
      ...
      self
        ...
        .then(function (rows) {
          self.putGuild(name).then(function (rows1) {
            ...
            self.putGuildMember(nguild_id, account.user_id, 1).then(function (rows2) {
              self.updateUserDiscountGoldForGuild(500000, account.user_id).then(function (rows2) {
  ...
  deleteGuildMember(user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('DELETE FROM guild_member WHERE UserId = ?', [user_id]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  leaveGuild(account) {
    ...
    return new Promise(function (resolve, reject) {
      self
        .deleteGuildMember(account.user_id)
        .then(function (rows) {
          data.complete = true
          return resolve(data)
        })
  ...
  deleteGuildMemberByIdGuild(guild_id, user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('DELETE FROM guild_member WHERE Id = ? and UserId = ?', [guild_id, user_id]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  getGuildById(guild_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT * FROM guild WHERE Id = ?', [guild_id]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
  ...
  joinGuild(account, guild_id) {
    ...
    return new Promise(function (resolve, reject) {
      self
        .getGuildById(guild_id)
        .then(function (rows) {
          self.putGuildMember(guild_id, account.user_id, 0).then(function (rows1) {
  ...
  updateGoldGpWinLossByIdAcc(win_gold, win_gp, is_win, is_loss, user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn
          .query('UPDATE users SET gold = gold + ?, gp = gp + ?, win = win + ?, loss = loss + ? WHERE IdAcc = ?', [
            win_gold,
            win_gp,
            is_win,
            is_loss,
  ...
  updateUser(account, callback) {
    ...
    return new Promise(function (resolve, reject) {
      self
        .updateGoldGpWinLossByIdAcc(account.player.win_gold, account.player.win_gp, account.player.is_win, account.player.is_loss, account.user_id)
        .then(function (rows) {
  ...
  getEventLogByIdAcc(user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT Event1, Event2 FROM event_log WHERE Id = ?', [user_id]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
  ...
  updateEventLogByIdAcc(scr, user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('UPDATE event_log SET ? WHERE Id = ?', [scr, user_id]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  updateGoldCashEventByIdAcc(gold, cash, user_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('UPDATE users SET cash = cash + ?, gold = gold + ? WHERE IdAcc = ?', [cash, gold, user_id]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows)
          else return reject()
        })
      ...
      self
        ...
        .then(function (rows) {
          ...
          if (update) {
            ...
            self.updateEventLogByIdAcc(sctp, account.user_id).then(function (rows2) {
              var xgold = 1000
              var xcash = 2000
              if (type === 0) {
              } else if (type === 3) {
              ...
              self.updateGoldCashEventByIdAcc(xgold, xcash, account.user_id).then(function (rows3) {
                rdata.complete = true
                rdata.gold = xgold
                rdata.cash = xcash
  ...
  getUserAvatarById(arr) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT * FROM user_avatars WHERE Id IN (?)', [arr]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
  ...
  updateUserAvatarEquipById(data, reg_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('UPDATE user_avatar_equiped SET ? WHERE Id = ?', [data, reg_id]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0 || rows[0].changedRows > 0) return resolve(rows)
          else return reject()
        })
    ...
    return new Promise(function (resolve, reject) {
      self
        ...
        .then(function (r_data) {
          ...
          self.updateUserAvatarEquipById(data, account.player.reg_id).then(function (rows) {
            rdata.complete = true
            return resolve(rdata)
          })
        })
  ...
  getAccountByFBId(id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('SELECT Id, Session FROM accounts where facebook_id = ?', [id]).then((rows) => {
          conn.release()
          if (rows[0].length > 0) return resolve(rows)
          else return reject()
        })
  ...
  putAccountFB(data) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('INSERT into accounts SET ?', [data]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  putUserFB(data) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('INSERT into users SET ?', [data]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  putAvatarUseFB(data) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn.query('INSERT into user_avatar_equiped SET ?', [data]).then((rows) => {
          conn.release()
          if (rows[0].affectedRows > 0) return resolve(rows)
          else return reject()
        })
  ...
  getUserByGameId(game_id) {
    ...
    return new Promise(function (resolve, reject) {
      self.connection.getConnection().then((conn) => {
        conn
          .query(
            'SELECT u.IdAcc, u.game_id, u.photo_url, u.bg_url, u.country, u.rank, u.gp, u.win, u.gender, u.loss FROM users u where LOWER(`game_id`) = ?',
            [game_id]
          )
          .then((rows) => {
            conn.release()
