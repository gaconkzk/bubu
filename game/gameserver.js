...
module.exports = class GameServer {
  constructor(id, maxPlayers, websocketserver) {
    ...
    if (process.env.vps === '1' || process.env.vps === '2') this.server_type = 0
    else {
      this.name = 'Boss'
      this.server_type = 0
    }
  ...
  pushBroadcastChannel(message) {
    ...
    for (var id in this.outgoingQueues) {
      ...
      if (account !== null && account.location_type === Types.LOCATION.CHANNEL) {
        if (Array.isArray(message) === true) this.outgoingQueues[id].push(message)
      ...
      if (this.outgoingQueues[id].length > 0) {
        ...
        if (account !== null && typeof account != 'undefined') {
          ...
          for (var i = 0; i < this.outgoingQueues[id].length; i++) {
            try {
              if (connection) connection.send(this.outgoingQueues[id][i])
            } catch (e) {}
          }
        }
