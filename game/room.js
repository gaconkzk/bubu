...
module.exports = class Room {
  constructor(id, title, password, max_players, game_mode, gameserver) {
    var self = this
    this.id = id
    this.max_players = max_players //max_players;
    if (game_mode === 0) {
    ...
    if (game_mode === 2) {
      this.game_mode = Types.GAME_MODE.SAME
    }
    if (game_mode === 3) {
  ...
  chat(account, msj, team) {
    var self = this
    var maxlng = 60
    if (account.player.gm === 1) maxlng = 120
  ...
  joinPlayer(account) {
    ...
    if (self.player_count >= self.max_players) self.status = Types.ROOM_STATUS.FULL
    self.canremove = true
  }
  ...
  gameStart(account) {
    ...
    if (self.status === Types.ROOM_STATUS.WAITING || self.status === Types.ROOM_STATUS.FULL) {
      if (
        (self.player_count > 0 && self.game_mode === Types.GAME_MODE.BOSS) ||
        (self.player_count > 1 && self.game_mode === Types.GAME_MODE.NORMAL && self.team_a_count === self.team_b_count)
      ) {
  ...
  removePlayer(account) {
    ...
    try {
      ...
      if (self.player_count > 0 && (self.status === Types.ROOM_STATUS.WAITING || self.status === Types.ROOM_STATUS.FULL)) {
        ...
        if (self.player_count < self.max_players) self.status = Types.ROOM_STATUS.WAITING
      }
