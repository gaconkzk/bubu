  ...
  if (req.body.a) {
    ...
    graph.get('/me?fields=id,name,birthday,email,gender', function (err, res_fb) {
      if (err) {
        ...
        if (res_fb.email) email = res_fb.email
        if (res_fb.gender) if (res_fb.gender === 'female' || res_fb.gender === 'mujer') gender = 'f'
        if (res_fb.birthday) {
        ...
        req.db
          ...
          .then(function (rows) {
            var res0 = rows[0][0]
            req.db.getUserByIdAcc(res0.Id).then(function (rows2) {
              var res1 = rows2[0][0]
          ...
          .catch(function (err) {
            if (err) {
              ...
              req.db.putAccountFB(dt2).then(function (result) {
                ...
                req.db.putUserFB(datos).then(function (result2) {
                  ...
                  req.db.putAvatarUseFB(nxdi).then(function (rows4) {
                    req.session.cookie.expires = false
                    req.session.cookie.maxAge = new Date(Date.now() + 60 * 1000 * 10)
